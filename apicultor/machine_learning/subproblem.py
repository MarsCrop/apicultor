...
def continuous_decode(features, targets, context, batch_size=5000, do_forward = False):
    ...
    for batch in range(batches):
        ...
        if len(np.mat(targets[bin:bend]).T) != batch_size:
            attended_targets = np.nan_to_num(targets)
        else:
            attended_targets = np.nan_to_num(np.mat(targets[bin:bend]).T)
        print("RUNNING ATTENTION FUNCTION USING")    
        print("CONTEXT", np.shape(context[bin:bend])) 
        print("FEATURES", np.shape(features))
        print("ATTENDED TARGETS", np.shape(attended_targets))               
        if np.shape(features) != np.shape(targets):
            attention_function = asyncio.run(parallel([1], 1, (context[bin:bend], np.nan_to_num(features[bin:bend]), attended_targets, 1.5, len(features)), func=decode, index = False, shared=False, fifo = False, lifc = False, continuous = batch))[0][0]
        else:
            attention_function = asyncio.run(parallel([1], 1, (context[bin:bend], np.nan_to_num(features), attended_targets, 1.5, len(features)), func=decode, index = False, shared=False, fifo = False, lifc = False, continuous = batch))[0][0]
        ...
        attending_features = features 
        if attention_function.shape[1] != attending_features.T.shape[0]:
            pass
        elif attention_function.shape[0] != attending_features.T.shape[0]:
            pass
        elif attention_function.shape[1] != attending_features.shape[0]:
            attending_features = attending_features[:attention_function.shape[1]].T
        if attention_function.shape[1] == 0:
            break
        print("ATTENTION FUNCTION SHAPE", np.shape(attention_function)) 
        print("FEATURES SHAPE", np.shape(attending_features)) 
