...
async def compute_feature_importance(dset, inst=0): 
    ...
    features 
    :param dset: it is a current list (set) of dataset + fine tunning retrieved information
...
async def forward_with_dropout(data_set, protected_groups=[], protected_lime = 0.8, importance_threshold = 0.2, protected_mse=0.2, error = [], protected_features = []):
    """
    Applies an importance-based "dropout" mechanism to feature weights and evaluates
    robustness and non-discrimination metrics on the processed dataset.

    The function loads pre-calculated importance metrics and MSE (Mean Squared Error),
    determines an importance threshold, and modifies the weights ('wset') of the
    dataset. It then uses 'continuous_decode' and 'validate_non_discrimination_and_robustness'
    functions to process the data and evaluate the robustness of the resulting model/output.

    The "dropout" is applied to zero out the weights of less important features based
    on a dynamic threshold, with a specific handling for protected groups.

    Args:
        data_set (tuple): A tuple containing the input data, labels, and a set of
                          weights/features (x, y, wset, ...).
        protected_groups (list, optional): A list of identifiers for the relevant
                                           protected groups. Defaults to `[]`.
        protected_lime (float, optional): Threshold/parameter related to LIME for
                                          protected groups. Defaults to `0.8`.
        importance_threshold (float, optional): Initial importance threshold for
                                                applying dropout. This value is
                                                overwritten by dynamically calculated
                                                thresholds from loaded data.
                                                Defaults to `0.2`.
        protected_mse (float, optional): Threshold/parameter related to MSE for
                                         protected groups. Defaults to `0.2`.
        error (list, optional): List for storing errors (currently not explicitly
                                used within the function body). Defaults to `[]`.
        protected_features (list, optional): List of features that should be handled
                                             specially for protected groups.
                                             Defaults to `[]`.
    
    Returns:
        numpy.ndarray: The decoded 'context' or the final processed result from the
                       'continuous_decode' function.
    
    Raises:
        FileNotFoundError: If 'IMPORTANT MSES.npy' or the importance files
                           (those with 'IMPORTANCE' in the name) are not found in
                           '/home/mc/sfs-python/sfs/'.

    Notes:
        - The function depends on NumPy files saved to disk ('IMPORTANT MSES.npy'
          and 'IMPORTANCE*.npy' files in the sfs directory).
        - It uses 'asyncio.run(parallel(...))' for parallel/asynchronous execution
          of 'continuous_decode', suggesting an environment that handles complex
          operations (like attention/sequence models).
        - The importance thresholds and dropout process indicate an implementation
          of techniques to mitigate bias or enhance model robustness based on
          explainability (LIME/feature importance).
    """
    ...
    res = await parallel([1], 1, (y[:min_len], x[:min_len], wset[:min_len], 250, True), func=continuous_decode, shared=False, index=False)
    print("RES:", res)
    attention_function, targets, attention_scores, weights, context = res[0][0], res[0][1], res[0][2], res[0][3], res[0][4]
    min_len = min(len(targets), len(context))
    #try:
    #    decoded_robustness = await validate_non_discrimination_and_robustness(targets, context, attention_scores, y, weights, 1e-4, 0)
    #except Exception as e:
    #    decoded_robustness = await validate_non_discrimination_and_robustness(targets[:min_len], context[:min_len], attention_scores[:min_len], y[:min_len], weights[:min_len], 1e-4, 0)
    #mse_decoded = mean_squared_error(targets, y)  
    #print("ROBUSTNESS:", decoded_robustness)
    return context, targets    

async def validate_non_discrimination_and_robustness(y, y_predicted, theta, x, proba, mse, statistical_parity_thresh=1e-4, local_parity_thresh = 0.2, class_parity_thresh = 0.2, conditional_parity_thresh = 0.2):
    """
    ...
    xcontext = await parallel([1], 1, (x, theta), func = context_vector, index = False, shared=False, fifo = False, lifc = False, continuous = False)
    xcontext = np.nan_to_num(xcontext[0])
    try:
